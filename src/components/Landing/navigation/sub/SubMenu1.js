...
import './border-css1.css' ;
const baseUrl = "./../../../images/landing/"
import text from './../../../../images/landing/text.png'
                ...
                <Paper className={classes.root} style={{marginLeft: 200, backgroundImage: `url(${bg})`}}>
                  <ClickAwayListener onClickAway={this.handleClose}>
                      <MenuList className={classes.container}>
                        <Grid container spacing={8} style={{padding: '10px 3%'}}>
                          {
                            this.props.subMenus.map((menu, index) => {
                              return <Grid item key={index} xs={12} style={{display: "flex", alignItems: "center", paddingLeft: '2.5vw !important', height: 120 }}>
                                        <Grid item xs={6} style={{display: "flex", position: 'relative', left: '-4%'}}>
                                          <div className={classes.buttonWrapper}>
                                            <MyButton icon={menu.icon} label={menu.label} href={menu.to || ""}/>                                      
                                          </div>
                                        </Grid>
                                        <Grid item xs={6} style={{ position: 'relative', left: '-9%'}}>
                                          <img style={{width: '119%'}} src={text}/>
                                        </Grid>
                                    </Grid>
                            })
                          }
                        ...
                        <Grid container spacing={8}>
                          {
                            this.props.commonMenus.map((menu, index) => {
                              return <Grid item key={index} xs={6} style={{paddingLeft: '2.5vw !important'}}>
